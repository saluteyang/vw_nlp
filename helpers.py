...
# plot sentiment with spline interpolation
def plotSentiment(sent_array, chunks, grain=300, save=False, savefile=None):
    x = np.linspace(1, chunks, num=chunks)
    y = sent_array
    f = interp1d(x, y, kind='cubic')
    xnew = np.linspace(1, chunks, num=grain)
...
# divide novel into parts
def chunks(word_list, num_chunks):
    div = []
    len_doc = len(word_list)
    chunk_size = len_doc//num_chunks + 1
    for i in range(0, len(word_list), chunk_size):
        div.append(word_list[i: i + chunk_size])
    return div
...
potential_stop_words = frozenset([
    "a", "about", "above", "across", "after", "afterwards", "again", "against",
    "all", "almost", "alone", "along", "already", "also", "although", "always",
    "am", "among", "amongst", "amoungst", "amount", "an", "and", "another",
    "any", "anyhow", "anyone", "anything", "anyway", "anywhere", "are",
